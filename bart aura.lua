-- Bart Tuff Hub (TELEPORT & CREDITS UPDATE)
...
local function RunIntro()
    ...
    IntroText.TextSize = 25 -- Ajustado para caber o texto maior
...
FooterRight.Position = UDim2.new(1, -120, 1, -22) -- Ajustado para o texto maior
...
local function CreateButton(name, callback)
    ...
    local active = false
    Button.MouseEnter:Connect(function() if not active then TweenService:Create(Button, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(20, 20, 20)}):Play() end end)
    Button.MouseLeave:Connect(function() if not active then TweenService:Create(Button, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(30, 30, 30)}):Play() end end)
    Button.MouseButton1Click:Connect(function()
        active = not active
        if active then UIGradient.Enabled = true TweenService:Create(Button, TweenInfo.new(0.5), {BackgroundColor3 = Color3.fromRGB(255, 255, 255)}):Play()
        else TweenService:Create(Button, TweenInfo.new(0.5), {BackgroundColor3 = Color3.fromRGB(30, 30, 30)}):Play() spawn(function() wait(0.5) if not active then UIGradient.Enabled = false end end) end
        callback(active)
    end)
    return Button
end
...
CreateButton("Teleporte Lobby", function()
    local root = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if root then
        -- Coordenadas genéricas de lobby (SpawnLocation principal)
...
CreateButton("Teleporte Mapa", function()
    local root = Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if root then
        -- Tenta achar um jogador longe do spawn para teleportar para o "mapa"
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= Players.LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
                if v.Character.HumanoidRootPart.Position.Magnitude > 150 then -- Longe do centro
